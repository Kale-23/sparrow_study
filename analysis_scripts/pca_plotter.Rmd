...
```{r, fig.height=15, fig.width=10, echo=FALSE, message=FALSE, error=FALSE}
# creating pca plots
...
for (j in 1:15) {
    for (i in 1:length(file_names)) {
        ...
        vect_out <- vectors |>
            select(colors, group) |>
            group_by(colors) |>
            summarise(val = length(unique(group))) |>
        ...
        vect_out2 <- vectors |>
            select(colors, group, short_basenames) |>
...
ifelse(line %% 2 == 0, {cyano = ifelse(last_var == "N", "Y", "N"); line = line + 1}, {choice = sample(choices, 1); last_var = choice; cyano = choice; line = line + 1})
